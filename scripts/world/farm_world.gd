...
const FARM_PLOT := Rect2(264, 226, 256, 176)
...
func _draw() -> void:
	...
	# Alternating grass grid creates readable pixel-art scale without textures.
	...
	# Tilled demonstration plot.
	draw_rect(FARM_PLOT, Color("76513d"), true)
	for x in range(int(FARM_PLOT.position.x) + 16, int(FARM_PLOT.end.x), 32):
		draw_line(Vector2(x, FARM_PLOT.position.y + 8), Vector2(x, FARM_PLOT.end.y - 8), Color("5e3d30"), 2.0)
	for y in range(int(FARM_PLOT.position.y) + 16, int(FARM_PLOT.end.y), 32):
		draw_line(Vector2(FARM_PLOT.position.x + 8, y), Vector2(FARM_PLOT.end.x - 8, y), Color("8e654d"), 1.0)
...
func _build_collision_geometry() -> void:
	# Four walls keep the player inside the farm.
	...
	# Farmhouse, pond, and trees are blocked for Phase A.
